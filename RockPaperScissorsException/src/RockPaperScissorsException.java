...
public class RockPaperScissorsException {
	// making the board a char array to accommodate X and O
		...
		private static void printBoard() {
			...
			// custom exceptions for out of bounds input
			if (Integer.parseInt(moves[0]) >= 3 || Integer.parseInt(moves[0]) < 0) {
				throw new TicTacException(moves[0]);
			}
			if (Integer.parseInt(moves[1]) >= 3 || Integer.parseInt(moves[1]) < 0) {
				throw new TicTacException(moves[0]);
			}
			gameOver = move(Integer.parseInt(moves[0]), Integer.parseInt(moves[1]));
			} catch (Exception e) {
				System.out.println("Pretty please input your move as X, Y:");
				printBoard();
			}
